...
def spendings_view(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            first_level_kind = form.cleaned_data['first_level_kind']
            second_level_kind = form.cleaned_data['second_level_kind']
            
            spendings_item_kind_to_add = SpendingKind.objects.get(first_level_kind=first_level_kind, second_level_kind=second_level_kind)
            spendings_item.kind = spendings_item_kind_to_add
            cash_kind_in_spending = CashKind.objects.get(id=form.cleaned_data['cash_kind_spending'])
    ...
    for p in spendingkind_objects:
        if p.first_level_kind not in structure_spendingkind.keys():
            second_levei_in_dict = [a.second_level_kind for a in spendingkind_objects.filter(first_level_kind=p.first_level_kind)]
